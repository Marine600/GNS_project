...
def conf_address(routeur):
    ...
    addresses["FastEthernet0/0"] = voisins[0][1][0:14] + f"{router_id[0:2]}/64"
    if len(voisins) >= 2 :
        addresses["GigabitEthernet1/0"] = voisins[1][1][0:14] + f"{router_id[0:2]}/64"

        if len(voisins) >= 3 :
            addresses["GigabitEthernet2/0"] = voisins[2][1][0:14] + f"{router_id[0:2]}/64"

            if len(voisins) == 4 :
                addresses["GigabitEthernet3/0"] = voisins[3][1][0:14] + f"{router_id[0:2]}/64"
